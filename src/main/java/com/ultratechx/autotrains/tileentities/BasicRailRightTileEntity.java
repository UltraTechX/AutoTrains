...
public class BasicRailRightTileEntity extends TileEntity{
			...
			for (int y = yCoord; y < yCoord + 1; y++){
				for (int z = zCoord - 1; z < zCoord + 2; z++){
					...
					if(tile != null && (tile instanceof BasicRailRightTileEntity)){
						if(this.isMaster()){
							if(((BasicRailRightTileEntity)tile).hasMaster()){
								i++;
								//System.out.println(i);
							}
						}else if(!((BasicRailRightTileEntity)tile).hasMaster()){
							//i++;
							//System.out.println(i);
						}
					}
		...
		for (int x = xCoord - 1; x < xCoord + 2; x++){
			for (int y = yCoord; y < yCoord + 1; y++){
				for (int z = zCoord - 1; z < zCoord + 2; z++){
					...
					if(tile != null && (tile instanceof BasicRailRightTileEntity)){
						((BasicRailRightTileEntity) tile).setMasterCoords(xCoord, yCoord, zCoord);
						((BasicRailRightTileEntity) tile).setHasMaster(true);
						((BasicRailRightTileEntity) tile).setIsMaster(master);
					}
				}
			}
		}
	...
	public boolean checkForMaster(){
		...
		return (tile != null && (tile instanceof BasicRailRightTileEntity));
	}
